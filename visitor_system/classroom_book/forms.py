    ...
    def clean(self):
        ...
        if start_time and end_time and classroom:
            ...
            if end_time - start_time > max_duration:
                self.add_error('end_time', 'Максимальная продолжительность бронирования - 8 часов')
            
            # Проверка доступности аудитории
            if not classroom.is_available(start_time, end_time):
                self.add_error('classroom', 'Аудитория уже забронирована на указанное время')
            
            # Проверка наличия доступного ключа
            available_key = classroom.get_available_key()
            if not available_key:
                self.add_error('classroom', 'Для этой аудитории нет доступных ключей')
